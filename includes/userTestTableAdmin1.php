<legend class="title-bold">
    <a onclick="hideUser();" id="usershide">
        <h4 class="glyphicon glyphicon-user"></h4> Потребители <span class="caret"></span> </a>
</legend>
<div class="row">
    <div class="col-xs-12 col-sm-12 col-md-12">
        <div class="form-group" id="users">
            <table class="table">
                <thead class="thead-dark">
                <tr>
                    <th scope="col"><span class="glyphicon glyphicon-star"></span></th>
                    <th scope="col"><span class="glyphicon glyphicon-user"></span> Име и фамилия</th>
                    <th scope="col"><span class="glyphicon glyphicon-user"></span> Псевдоним</th>
                    <th scope="col"><span class="glyphicon glyphicon-envelope"></span> Имейл</th>
                    <th scope="col" class="text-center"><span class="glyphicon glyphicon-calendar"></span> Дата на
                        регистрация
                    </th>
                    <th scope="col" class="text-center"><span class="glyphicon glyphicon-calendar"></span> Последно
                        влизане
                    </th>
                    <th scope="col" class="text-center"><span class="glyphicon glyphicon-king"></span> Админ. права</th>
                    <th scope="col" class="text-center"><span class="glyphicon glyphicon-erase"></span> Изтриване</th>
                </tr>
                </thead>
                <tbody>
